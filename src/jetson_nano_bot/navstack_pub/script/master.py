...
def predict(text):
   ...
   filtered_predictions = predictions_flat[predictions_flat > 0.2]
   ...
   if len(top_positions) == 0 or (len(top_positions) == 1 and 'Viêm nhiễm phụ khoa' in list_ten_benh[top_positions[0]]):
      ...
      robot_state = 2
   return answer
def extract_disease_ordered(text, disease_to_clinic):
    # Create a pattern that matches any disease in the dictionary
    pattern = '|'.join(re.escape(disease) for disease in disease_to_clinic.keys())
    # Find all occurrences of the diseases in the text in order
    matches = re.finditer(pattern, text)
    # Extract matched diseases in order
    diseases = [match.group(0) for match in matches]
    return diseases

def get_clinics_to_visit(diseases, disease_to_clinic):
    clinics = []
    for disease in diseases:
        clinic = disease_to_clinic.get(disease)
...
def recommend_clinics(input_string):
   ...
   diseases = extract_disease_ordered(input_string, disease_to_clinic)
   print("diseases  ",diseases)
   clinics_to_visit = get_clinics_to_visit(diseases, disease_to_clinic)
   print("clinics_to_visit  ",clinics_to_visit)
   # Định dạng chuỗi kết quả
   if clinics_to_visit != []:
      #print(clinics_to_visit)
      ...
      output_string = "Không có thông tin phòng khám cho các bệnh được nhắc đến trong yêu cầu của bạn."
...
def set_goal_based_on_text(text):
   ...
   destinations = {
      "Phòng khám hô hấp": {"x": 0.543, "y": 8.4735, "w": 1.0},
      "Phòng khám nội tiết": {"x": 0.94255, "y": -6.431, "w": 1.0},
      "Phòng khám tim mạch": {"x": 24.389, "y": -0.3, "w": 1.0},
      "Phòng khám cơ xương khớp": {"x": 0.0, "y": 0.0, "w": 1.0},
      "Phòng khám tiêu hóa": {"x": 2.0, "y": -11.728, "w": 1.0},
...
def main():
	...
	while True:
		...
		responds = ""
		listen_result =  get_text()
		if listen_result is not None:
			listen_result = listen_result.lower()
		else:
			listen_result = ""
		chat_respond = bot.respond(listen_result)
		chat_respond = chat_respond.lower()
		print(chat_respond)
		if "Xin lỗi, tôi không hiểu bạn đang nói gì. bạn có thế nói lại không?" != chat_respond:
			if state ==0:#khoi dau
				predict_result = ""
				if chat_respond == "lead the way":
					state = 3
					responds = set_goal_based_on_text(listen_result)
					if "not valid" == responds:
						speak("Bạn muốn đến phòng khám nào ạ?")
				elif chat_respond == "say hi":
					speak("xin chào quý khách, tôi là charon , một robot trợ giúp khám bệnh.. xin hỏi quý khách cần gì ạ?")
					state = 1
					print("state", state)
				elif chat_respond == "kham benh":
					speak("quý khách vui lòng nêu ra những triệu chứng mình đang gặp phải nhé!")
					state = 2
			elif state ==1:#chao hoi xong
				if chat_respond == "say Hi":
					speak("xin chào!")
				elif chat_respond == "kham benh":
					speak("quý khách vui lòng nêu ra những triệu chứng mình đang gặp phải nhé!")
					state = 2
				elif chat_respond == "lead the way":
					state = 3
					responds = set_goal_based_on_text(listen_result)
					if "not valid" == responds:
						speak("Bạn muốn đến phòng khám nào ạ?")
					...
					if counter_giupgi == 0:
						speak("Tôi có thể giúp gì cho bạn ạ?")
						counter_giupgi=counter_giupgi+1
			elif state == 2:
				predict_result = predict(listen_result)
				if again:
					if chat_respond == "no":
						state =1
						again_in = True
		
				if "qua dữ liệu trên, tôi chuẩn đoán sơ bộ được" in predict_result:
					speak(predict_result)
					print("listen_result  ",listen_result)
					speak_phong_kham,phong_kham =recommend_clinics(predict_result)
					speak(speak_phong_kham)
					again = False
					state = 3
					responds = set_goal_based_on_text(speak_phong_kham)
				elif chat_respond == "accepted":
					speak("quý khách xui lòng nêu ra những triệu chứng mình đang gặp phải nhé!")
				else:
					speak("Xin lỗi, dựa trên dữ kiệu bạn cung cấp, tôi chưa thể chuẩn đoán được. bạn có thể cung cấp lại không ạ?")
					again = True
			elif state == 3:
				if "chúng ta đã đến" in responds:
					state =0
				else:
					state = 1
				
		else:
			speak("Xin lỗi, tôi không hiểu bạn đang nói gì. bạn có thế nói lại không?")
